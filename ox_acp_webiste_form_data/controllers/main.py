...
class WebsiteFormController(http.Controller):
    ...
    @http.route('/api/submit_form', auth='public', website=False, type='http', methods=["POST", "OPTIONS"], csrf=False)
    def submit_form(self, **kwargs):
        try:
            ...
            if existing_partner:
                return request.make_response(json.dumps({
                    'success': False,
                    'error': "This email address is already registered. Please use the Membership Portal to log in or reset your password."
                }), headers=RESPONSE_HEADERS)
            ...
            try:
                ...
                # 4. Create portal user using portal wizard
                ...
                # 5. Retrieve membership number (always available from your override)
                membership_number = new_partner.membership_number_auto
                ...
                logger.error(f"Error creating partner/user: {str(e)}", exc_info=True)
                return request.make_response(json.dumps({
                    'success': False,
                    'error': str(e),
                    'message': "Error while creating partner or portal user"
                }), headers=RESPONSE_HEADERS)
